...
func OpenKV[K KType, V any](rr *ReducerRuntime, kvName string) SortedKV[K, V] {
	...
	rr.kvMaps[kvName] = kvMapData{
		...
		writeToDb: func(writeTx *bolt.Tx) error {
			...
			reducerBucket := glob.Bucket([]byte(rr.reducerName))
			kvParent, err := reducerBucket.CreateBucketIfNotExists(kvBucket)

			if err != nil {
				return err
			}

			bucket, err = kvParent.CreateBucketIfNotExists([]byte(kvName))
			if err != nil {
				return err
			}

			if err := bp.WriteProxyToDb(bucket); err != nil {
				return err
			}
